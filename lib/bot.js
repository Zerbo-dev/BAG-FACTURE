...
function paymentModeKeyboard() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("🏦 Virement bancaire", "paymode_bank")],
    [Markup.button.callback("📱 Orange Money", "paymode_om")],
    [Markup.button.callback("💵 Espèces", "paymode_cash")],
  ]);
}
...
async function askPaymentMode(ctx, state) {
  ...
  await setState(ctx.chat.id, state);
  await ctx.reply("Quel est le mode de paiement pour cette facture ?", paymentModeKeyboard());
}
...
async function showPreview(ctx, state) {
  ...
  const paymentLine = state.paymentMode ? `Paiement : ${state.paymentMode}\n` : "";
 ...
 * l'ordre : mode de paiement (facture seulement), puis termes, puis
...
async function afterItemsCollected(ctx, state) {
  if (state.docType === "facture" && !state.paymentMode) {
...
bot.action(/^paymode_(bank|om|cash)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const state = await getState(ctx.chat.id);
  if (!state) return;
  state.paymentMode = PAYMENT_MODES[ctx.match[1]];
  await setState(ctx.chat.id, state);
...
bot.on("text", async (ctx) => {
  ...
  switch (state.step) {
    ...
    case "await_payment_mode":
      await ctx.reply("Utilise les boutons ci-dessus pour choisir le mode de paiement 👆");
      break;
