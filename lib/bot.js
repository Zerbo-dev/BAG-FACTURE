...
const MAX_ITEMS = 40; // garde-fou anti-abus ; le document se pagine automatiquement au-delà d'une page
...
function paymentModeKeyboard(selected) {
  const icons = { bank: "🏦", om: "📱", cash: "💵" };
  const rows = Object.entries(PAYMENT_MODES).map(([key, label]) => {
    const checked = selected.includes(label) ? "✅ " : "";
    return [Markup.button.callback(`${checked}${icons[key]} ${label}`, `paymode_toggle_${key}`)];
  });
  rows.push([Markup.button.callback("☑️ Valider la sélection", "paymode_validate")]);
  return Markup.inlineKeyboard(rows);
}
...
async function askPaymentMode(ctx, state) {
  state.step = "await_payment_mode";
  state.paymentModes = [];
  await setState(ctx.chat.id, state);
  await ctx.reply(
    "Quel(s) mode(s) de paiement pour cette facture ? Tu peux en choisir plusieurs.",
    paymentModeKeyboard(state.paymentModes)
  );
}
...
async function showPreview(ctx, state) {
  ...
  const paymentLine =
    state.paymentModes && state.paymentModes.length
      ? `Paiement : ${state.paymentModes.join(", ")}\n`
      : "";
 ...
 * l'ordre : mode(s) de paiement (facture seulement), puis termes, puis
 * garantie (ces deux dernieres pour tous les types de document), et on
 * passe a l'apercu une fois tout renseigne.
 */
async function afterItemsCollected(ctx, state) {
  if (state.docType === "facture" && state.paymentModes === null) {
    await askPaymentMode(ctx, state);
    return;
  }
...
bot.action("add_item", async (ctx) => {
  ...
  if (state.items.length >= MAX_ITEMS) {
    return ctx.reply(
      `Limite de ${MAX_ITEMS} prestations atteinte pour cette commande. ` +
...
bot.action(/^paymode_toggle_(bank|om|cash)$/, async (ctx) => {
  const state = await getState(ctx.chat.id);
  if (!state) return ctx.answerCbQuery();

  const label = PAYMENT_MODES[ctx.match[1]];
  const idx = state.paymentModes.indexOf(label);
  if (idx === -1) {
    state.paymentModes.push(label);
  } else {
    state.paymentModes.splice(idx, 1);
  }
  await setState(ctx.chat.id, state);
  await ctx.answerCbQuery(idx === -1 ? `${label} ajouté` : `${label} retiré`);
  await ctx.editMessageReplyMarkup(paymentModeKeyboard(state.paymentModes).reply_markup).catch(() => {});
});

bot.action("paymode_validate", async (ctx) => {
  const state = await getState(ctx.chat.id);
  if (!state) return ctx.answerCbQuery();

  if (!state.paymentModes || state.paymentModes.length === 0) {
    await ctx.answerCbQuery("Choisis au moins un mode de paiement.", { show_alert: true });
    return;
  }
  await ctx.answerCbQuery();
...
bot.on("text", async (ctx) => {
  ...
  switch (state.step) {
    ...
    case "await_payment_mode":
      await ctx.reply("Utilise les boutons ci-dessus pour choisir un ou plusieurs modes, puis \"Valider\" 👆");
      break;
