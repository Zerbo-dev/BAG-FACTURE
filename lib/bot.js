...
bot.action("confirm", async (ctx) => {
  ...
  try {
    const number = await nextDocNumber(state.docType);
    ...
    await ctx.replyWithPhoto({ source: pngBuffer, filename: `${fileBase}.png` });
    await ctx.replyWithDocument({ source: pdfBuffer, filename: `${fileBase}.pdf` });
    await ctx.reply(`✅ ${label} n° ${number} généré(e) avec succès !`);

    await clearState(ctx.chat.id);
  } catch (err) {
    console.error(err);
    await ctx.reply("❌ Une erreur est survenue pendant la génération. Réessaie avec /devis ou /facture.");
  } finally {
